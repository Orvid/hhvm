...
#define RCC_I(class_name, const_name, value) \
    ...
    RCC_I(PDO, PARAM_BOOL, PDO_PARAM_BOOL);
    RCC_I(PDO, PARAM_NULL, PDO_PARAM_NULL);
    RCC_I(PDO, PARAM_INT, PDO_PARAM_INT);
    RCC_I(PDO, PARAM_STR, PDO_PARAM_STR);
    RCC_I(PDO, PARAM_LOB, PDO_PARAM_LOB);
    RCC_I(PDO, PARAM_STMT, PDO_PARAM_STMT);
    RCC_I(PDO, PARAM_INPUT_OUTPUT, PDO_PARAM_INPUT_OUTPUT);

    RCC_I(PDO, PARAM_EVT_ALLOC, PDO_PARAM_EVT_ALLOC);
    RCC_I(PDO, PARAM_EVT_FREE, PDO_PARAM_EVT_FREE);
    RCC_I(PDO, PARAM_EVT_EXEC_PRE, PDO_PARAM_EVT_EXEC_PRE);
    RCC_I(PDO, PARAM_EVT_EXEC_POST, PDO_PARAM_EVT_EXEC_POST);
    RCC_I(PDO, PARAM_EVT_FETCH_PRE, PDO_PARAM_EVT_FETCH_PRE);
    RCC_I(PDO, PARAM_EVT_FETCH_POST, PDO_PARAM_EVT_FETCH_POST);
    RCC_I(PDO, PARAM_EVT_NORMALIZE, PDO_PARAM_EVT_NORMALIZE);

    RCC_I(PDO, FETCH_USE_DEFAULT, PDO_FETCH_USE_DEFAULT);
    RCC_I(PDO, FETCH_LAZY, PDO_FETCH_LAZY);
    RCC_I(PDO, FETCH_ASSOC, PDO_FETCH_ASSOC);
    RCC_I(PDO, FETCH_NUM, PDO_FETCH_NUM);
    RCC_I(PDO, FETCH_BOTH, PDO_FETCH_BOTH);
    RCC_I(PDO, FETCH_OBJ, PDO_FETCH_OBJ);
    RCC_I(PDO, FETCH_BOUND, PDO_FETCH_BOUND);
    RCC_I(PDO, FETCH_COLUMN, PDO_FETCH_COLUMN);
    RCC_I(PDO, FETCH_CLASS, PDO_FETCH_CLASS);
    RCC_I(PDO, FETCH_INTO, PDO_FETCH_INTO);
    RCC_I(PDO, FETCH_FUNC, PDO_FETCH_FUNC);
    RCC_I(PDO, FETCH_GROUP, PDO_FETCH_GROUP);
    RCC_I(PDO, FETCH_UNIQUE, PDO_FETCH_UNIQUE);
    RCC_I(PDO, FETCH_KEY_PAIR, PDO_FETCH_KEY_PAIR);
    RCC_I(PDO, FETCH_CLASSTYPE, PDO_FETCH_CLASSTYPE);
    RCC_I(PDO, FETCH_SERIALIZE, PDO_FETCH_SERIALIZE);
    RCC_I(PDO, FETCH_PROPS_LATE, PDO_FETCH_PROPS_LATE);
    RCC_I(PDO, FETCH_NAMED, PDO_FETCH_NAMED);

    RCC_I(PDO, ATTR_AUTOCOMMIT, PDO_ATTR_AUTOCOMMIT);
    RCC_I(PDO, ATTR_PREFETCH, PDO_ATTR_PREFETCH);
    RCC_I(PDO, ATTR_TIMEOUT, PDO_ATTR_TIMEOUT);
    RCC_I(PDO, ATTR_ERRMODE, PDO_ATTR_ERRMODE);
    RCC_I(PDO, ATTR_SERVER_VERSION, PDO_ATTR_SERVER_VERSION);
    RCC_I(PDO, ATTR_CLIENT_VERSION, PDO_ATTR_CLIENT_VERSION);
    RCC_I(PDO, ATTR_SERVER_INFO, PDO_ATTR_SERVER_INFO);
    RCC_I(PDO, ATTR_CONNECTION_STATUS, PDO_ATTR_CONNECTION_STATUS);
    RCC_I(PDO, ATTR_CASE, PDO_ATTR_CASE);
    RCC_I(PDO, ATTR_CURSOR_NAME, PDO_ATTR_CURSOR_NAME);
    RCC_I(PDO, ATTR_CURSOR, PDO_ATTR_CURSOR);
    RCC_I(PDO, ATTR_ORACLE_NULLS, PDO_ATTR_ORACLE_NULLS);
    RCC_I(PDO, ATTR_PERSISTENT, PDO_ATTR_PERSISTENT);
    RCC_I(PDO, ATTR_STATEMENT_CLASS, PDO_ATTR_STATEMENT_CLASS);
    RCC_I(PDO, ATTR_FETCH_TABLE_NAMES, PDO_ATTR_FETCH_TABLE_NAMES);
    RCC_I(PDO, ATTR_FETCH_CATALOG_NAMES, PDO_ATTR_FETCH_CATALOG_NAMES);
    RCC_I(PDO, ATTR_DRIVER_NAME, PDO_ATTR_DRIVER_NAME);
    RCC_I(PDO, ATTR_STRINGIFY_FETCHES, PDO_ATTR_STRINGIFY_FETCHES);
    RCC_I(PDO, ATTR_MAX_COLUMN_LEN, PDO_ATTR_MAX_COLUMN_LEN);
    RCC_I(PDO, ATTR_EMULATE_PREPARES, PDO_ATTR_EMULATE_PREPARES);
    RCC_I(PDO, ATTR_DEFAULT_FETCH_MODE, PDO_ATTR_DEFAULT_FETCH_MODE);

    RCC_I(PDO, ERRMODE_SILENT, PDO_ERRMODE_SILENT);
    RCC_I(PDO, ERRMODE_WARNING, PDO_ERRMODE_WARNING);
    RCC_I(PDO, ERRMODE_EXCEPTION, PDO_ERRMODE_EXCEPTION);

    RCC_I(PDO, CASE_NATURAL, PDO_CASE_NATURAL);
    RCC_I(PDO, CASE_LOWER, PDO_CASE_LOWER);
    RCC_I(PDO, CASE_UPPER, PDO_CASE_UPPER);

    RCC_I(PDO, NULL_NATURAL, PDO_NULL_NATURAL);
    RCC_I(PDO, NULL_EMPTY_STRING, PDO_NULL_EMPTY_STRING);
    RCC_I(PDO, NULL_TO_STRING, PDO_NULL_TO_STRING);

    RCC_I(PDO, FETCH_ORI_NEXT, PDO_FETCH_ORI_NEXT);
    RCC_I(PDO, FETCH_ORI_PRIOR, PDO_FETCH_ORI_PRIOR);
    RCC_I(PDO, FETCH_ORI_FIRST, PDO_FETCH_ORI_FIRST);
    RCC_I(PDO, FETCH_ORI_LAST, PDO_FETCH_ORI_LAST);
    RCC_I(PDO, FETCH_ORI_ABS, PDO_FETCH_ORI_ABS);
    RCC_I(PDO, FETCH_ORI_REL, PDO_FETCH_ORI_REL);

    RCC_I(PDO, CURSOR_FWDONLY, PDO_CURSOR_FWDONLY);
    RCC_I(PDO, CURSOR_SCROLL, PDO_CURSOR_SCROLL);

    Native::registerClassConstant<KindOfStaticString>(
      s_PDO.get(),
      makeStaticString("ERR_NONE"),
      makeStaticString(PDO_ERR_NONE)
    );
